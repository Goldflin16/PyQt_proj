...
def new_note():
    name_note = QInputDialog().getText(QInputDialog(), "Введення назви замітки", "Введіть назву замітки")[0]
...
def new_tag():
    try:
        name_note = note_list_widget.currentItem().text()
        name_tag = QInputDialog().getText(QInputDialog(), "Введіть назви тега", "Введіть назву тега")[0]
        data[name_note]["TAG"].append(name_tag)
        write_json(name_file, data)
        tag_list_widget.addItem(name_tag)
    except:
        error("Щоб створити тег, потрібно вибрати замітку")
...
def drop_tag():
    try:
        ...
        error("Щоб видалити тег, спочатку виберіть замітку")
